      ...
      .map((plan: ManufacturingPlan) => {
        ...
        const bomItems = (bomData || [])
          ...
          .map((item) => {
            ...
            const subtotal = costPrice * item.qty_per_unit * plan.quantity;
      ...
      .forEach((plan: ManufacturingPlan) => {
        ...
        bomItems.forEach((item) => {
          ...
          if (aggregatedMap.has(key)) {
            ...
            existing.total_qty += item.qty_per_unit * plan.quantity;
            existing.total_cost += costPrice * item.qty_per_unit * plan.quantity;
          } else {
            const stockQty = partInfo.product_code 
              ? stocksMap.get(partInfo.product_code) || 0 
              : 0;
            const totalQty = item.qty_per_unit * plan.quantity;
